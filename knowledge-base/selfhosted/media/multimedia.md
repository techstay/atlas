...
编辑 `docker-compose.yml`，在 `immich-server` 服务中取消注释 `extends` 部分，并将 `cpu` 改为对应的后端：

```yaml
  immich-server:
    extends:
      file: hwaccel.transcoding.yml
      service: nvenc # 可选 nvenc / quicksync / rkmpp / vaapi / vaapi-wsl
```

Docker 配置完成后，还需要在 Immich 管理后台启用硬件加速：

1. 登录 Immich → **Administration** → **Video Transcoding Settings**
2. **Hardware Acceleration** 选择对应的后端（NVIDIA 选 `NVENC`，Intel 选 `Quick Sync`，AMD 选 `VAAPI`）
3. **Target Video Codec** 建议选择 `HEVC` 或 `AV1`（NVIDIA RTX 40 系列支持 AV1，效率更高）
4. 可选：启用 **Hardware Decoding** 实现端到端硬件加速
5. 保存设置

> ⚠️ NVIDIA 和 AMD 显卡不支持 VP9 编码，不要选择 VP9 作为目标编码。

### 机器学习硬件加速 🧠

为 AI 功能（人脸识别、智能搜索等）启用 GPU 加速。编辑 `docker-compose.yml`，在 `immich-machine-learning` 服务中修改两处：

1. `image` 标签末尾添加 `-cuda`（NVIDIA 选 `cuda`，Intel 选 `openvino`，AMD 选 `rocm`）
2. 取消注释 `extends` 部分，`service` 设为对应后端：
...
验证加速是否生效：查看 ML 容器日志，应包含 `CUDAExecutionProvider` 或 `TensorrtExecutionProvider`。

> ⚠️ CUDA 版本的 ML 镜像较大（约 1.2GB），首次拉取需要较长时间。启动时模型加载也可能导致健康检查暂时失败，等待几分钟即可恢复正常。

### 其他说明 📝

**浏览器兼容性 🌐** 推荐使用 **Edge** 或 **Chrome** 浏览器访问 Immich。Firefox 不支持 HEVC 编码，在浏览相册视频时可能无法正常播放。
